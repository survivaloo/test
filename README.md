...
本格的な囲碁が遊べるWebアプリケーションです。2つのバージョンを用意しています：

### 🚀 AlphaZero-lite版（9×9）【NEW!】

**機械学習で自己進化するAI**を搭載した最新バージョン！

**すぐにプレイ（HTMLプレビュー）:**
```
https://htmlpreview.github.io/?https://github.com/survivaloo/test/blob/claude/deploy-go-game-tunnel-fuVPB/go-game/alphazero-lite.html
```

#### 🧠 AlphaZero-lite版の特徴

- **自己対局による学習**: AIが自分自身と対戦して強くなります
- **ニューラルネットワーク**: 7層の入力特徴量（自石・敵石・手番・アタリ・呼吸点など）
- **タクティカル分析**: 取り・防御・アタリを自動判定
- **MCTS（モンテカルロ木探索）**: 5〜60回のシミュレーション（調整可能）
- **ハイブリッドポリシー**: NN（40%）+ タクティカル（60%）のブレンド
- **統計表示**: 対局数・勝率・平均手数・学習回数・Loss値
- **モデル保存**: ブラウザのlocalStorageに学習済みモデルを保存
- **ダークテーマ**: 美しい黒背景UI
- **完全ゼロ依存**: 外部ライブラリ不要、1ファイルで完結

### 🎯 クラシック版（19×19）

**定石データベース＋高度評価関数**による強力なAI
...
- 🤖 **超強化AI対戦モード**（プロ棋士レベルの思考エンジン搭載！）
  - **1. 深化ミニマックス探索（深さ3-4）**
    ...
    - 劣勢時は自動で深さ4読みに切り替え
  - **2. 強化モンテカルロ木探索（MCTS）**
    - 中盤の複雑な局面で150回ランダムシミュレーション（3倍強化）
    - 統計的に最良の手を選択
    - 盤面の25-75%埋まった時に発動（適用範囲拡大）
  - **3. 拡張定石データベース**
  ...
  - **4. ヨセ（終盤）専用ロジック**
    ...
    - ダメ詰め・辺の1目を正確に評価
  - **超高度な評価関数**
    - 相手の石を取る手を最優先（優先度200+、重み1.5倍強化）
    - アタリを掛ける攻撃手（優先度160+、重み1.3倍強化）
    - 自分の石の防御（優先度180+、重み1.3倍強化）
    - 1呼吸点の石を守る重要性を大幅強化（50倍評価）
    - 石の連絡と切断の判断
    - テリトリーの拡大
    - 位置の価値評価（隅 > 辺 > 中央）
  - **形勢判断システム**
